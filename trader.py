...
class Trader(threading.Thread):
        ...
        if self.quote_history_file and os.path.isfile(self.quote_history_file):
            ...
            if content:
                ...
                if len(fields) > len(self._csv_fields):
                    ...
                    mapping_fields = self._csv_fields + [ 'nan%s' % i for i in range(fill_length) ]

                self.mapping_fields = mapping_fields
                self.dataframe = pandas.io.parsers.read_csv( StringIO(content), names=mapping_fields, parse_dates= [[0,1]])
    ...
    def run(self):
        ...
        self.check_strategy()
        self.read_data()
        minute = datetime.datetime.now().strftime('%M') # one minute poor man timmer
        ...
        if self.quote_now_file :
            ...
            while self.status:
                ...
                self.ol = OPEN = Open = O = ol = self.dataframe.open.values[-backbars:]
                self.hl = HIGH = High = H = hl  = self.dataframe.high.values[-backbars:]
                self.ll = LOW = Low = L = ll  = self.dataframe.low.values[-backbars:]
                self.cl = CLOSE = Close = C = cl = self.dataframe.close.values[-backbars:]
    ...
    def quote(self):
        ...
        try:
            ...
            dt = datetime.datetime.strptime(dt_string, '%Y/%m/%d %H:%M') # FIXME able to support %Y-%m-%d
            Open = price[0]
            High = price[1]
            Low = price[2]
            Close = price[3]
