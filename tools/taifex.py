...
def main(start_date, ym='', download=None, auto=None, final_code=None, directory=None):
    ...
    while start_date <= today:
        filename = data_file % ( start_date.year, start_date.month, start_date.day)
        filename_path = os.path.join(taifex_dir, filename)
        filename_path_holder = filename_path[:-3] + 'html' # replace .zip to .html
        remote_filename = url_file %  ( start_date.year, start_date.month, start_date.day)
        ...
        if not os.path.isfile(filename_path):
            ...
            not_file_holder_exist  = (not os.path.isfile(filename_path_holder) and start_date < today)
            ...
            for fn in zfile.namelist():
                if fn.endswith(".rpt"):
                    ...
                    for row in reader:
                        ...
                        try:
                            ...
                            if Ym == ym and Code in commodity_codes:
                                line = "%s %s,%s,%s" % (Date, Time, Price, Volume)
                                #sys.exit()
                                db[Code].append(line)
                    ...
                    #for code in ['TF']:
                        ...
                        next_close_time = start_date_time + datetime.timedelta(seconds=60)
                                ...
                                if dt >= next_close_time and pre_dt < next_close_time:
                                    ...
                                    if dt >= next_next_time:
                                        while dt >= next_next_time:
                                            line_list = [next_close_time, oo, hh, ll, cc, vv]
                                            #print line_list
                        ...
                        for idx, line_list in enumerate(min1):
                            ...
                            for fn,v in files.items():
                                if v == 1:
                                    dt = line_list[0].strftime("%Y/%m/%d,%H:%M")
...
if __name__ == '__main__':
    ...
    if args.start and args.ym or args.auto or (args.download and args.start):
        main(args.start, args.ym, args.download , args.auto, args.code, args.dir)
